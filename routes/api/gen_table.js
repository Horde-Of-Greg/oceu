...
function generate_table(outputs, flags) {
  ...
  const needs_chance = outputs[0].chance,
    ...
    rates_flag = util.find_flag(flags, "--rates"),
    input_flag = util.parse_flag(flags, "--input"),
    output_flag = util.parse_flag(flags, "--output");
  machine_count = util.parse_flag(flags, "--count") || 1;
  ...
  outputs.forEach((row) => {
    ...
    if (rates_flag) {
      if (input_flag) {
        row.input_rates = util.calculate_rates(row, input_flag * machine_count);
        input_length = Math.max(row.input_rates.toString().length, input_length);
      }
      row.rates = util.calculate_rates(row, (output_flag || 1) * machine_count);
  ...
  let table = outputs
    .map((row) => {
      ...
      if (util.find_flag(flags, "--tick") || row.time < 20) {
      ...
      if (flags.includes("--ce") && row.tier == 9) {
        ...
        entry += generate_entry(util.get_tier_name(row.tier), "", "", tier_length);
      }
