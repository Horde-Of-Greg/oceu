...
function calculate_overclock(recipe, voltage) {
  ...
  const is_ce = util.find_flag(recipe.flags, "--ce");

  if (util.find_flag(recipe.flags, "--parallel")) {
    const parallel = Math.min(
      recipe.base_parallel,
      Math.floor((recipe.amperage * util.get_eu_t(voltage)) / recipe.base_eu),
    );
    base_eu = recipe.base_eu * parallel;
    output.parallel = parallel;
  }

  let overclock_tiers = Math.max(0, voltage - util.get_voltage_tier(base_eu, is_ce));
  ...
  if (is_ce) {
    ...
    let doubling_count;
    if (util.find_flag(recipe.flags, "--macerator")) {
      // Macerators in CE only start scaling in chanced outputs after MV
      doubling_count = voltage >= util.get_voltage_from_name("MV") ? overclock_tiers - 1 : overclock_tiers;
    } else {
      // CE fuckery
      doubling_count = voltage >= util.get_voltage_from_name("MV") ? overclock_tiers + 1 : overclock_tiers;
    }
    ...
    let effective_oc;
    if (util.find_flag(recipe.flags, "--lcr")) {
  ...
  const time_multiplier = parseFloat(util.parse_flag(recipe.flags, "--time"));
  const eu_multiplier = parseFloat(util.parse_flag(recipe.flags, "--eu"));
  if (time_multiplier) {
...
function calculate_ebf_overclock(recipe, voltage) {
  ...
  if (util.find_flag(recipe.flags, "--ce")) {
  ...
  let eu_tier = util.get_voltage_tier(
    base_eu * ebf_eu_discount(recipe.base_recipe_heat, recipe.base_coil_heat),
    false,
  );

  const recipe_voltage = util.get_voltage_tier(
    util.get_eu_t(voltage) * recipe.amperage - 1,
    false,
  );
...
function run_recipe(recipe) {
  const eu_tier = util.get_voltage_tier(
    recipe.base_eu,
