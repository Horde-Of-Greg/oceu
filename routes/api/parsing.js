...
function parse_input(input) {
	...
	if (util.find_flag(flags, "--input") || util.find_flag(flags, "--output")) {
		flags.push("--rates");
	}

	for (let i = 0; i < util.voltage_tiers.length; i++) {
		if (
			util.find_flag(flags, `--${util.voltage_tiers[i].name.toLowerCase()}`)
		) {
			flags.push(`--voltage:${util.voltage_tiers[i].name.toLowerCase()}`);
		}
	}
	...
	if (flags.includes("--rf")) {
		output.base_eu = Math.floor(output.base_eu / 4);
	}
